...
def year_crumb(date):
    ...
    return Crumb(year, reverse("zinnia:entry_archive_year", args=[year]))
...
def day_crumb(date):
    ...
    month = date.strftime("%m")
    day = date.strftime("%d")
    return Crumb(day, reverse("zinnia:entry_archive_day", args=[year, month, day]))
...
MODEL_BREADCRUMBS = {
    "Tag": lambda x: [Crumb(_("Tags"), reverse("zinnia:tag_list")), Crumb(x.name)],
    "Author": lambda x: [Crumb(_("Authors"), reverse("zinnia:author_list")), Crumb(x.__str__())],
    "Category": lambda x: [Crumb(_("Categories"), reverse("zinnia:category_list"))]
    + [Crumb(anc.__str__(), anc.get_absolute_url()) for anc in x.get_ancestors()]
    + [Crumb(x.title)],
    "Entry": entry_breadcrumbs,
}

ARCHIVE_REGEXP = re.compile(r".*(?P<year>\d{4})/(?P<month>\d{2})?/(?P<day>\d{2})?.*")

ARCHIVE_WEEK_REGEXP = re.compile(r".*(?P<year>\d{4})/week/(?P<week>\d+)?.*")
...
def handle_page_crumb(func):
    ...
    def wrapper(path, model, page, root_name):
        ...
        if page and page.number > 1 and breadcrumbs:
...
def retrieve_breadcrumbs(path, model_instance, root_name=""):
    ...
    if date_match := ARCHIVE_WEEK_REGEXP.match(path):
        ...
        date_breadcrumbs = [year_crumb(year_date), Crumb(_("Week %s") % week)]
    ...
    url_components = [comp for comp in path.replace(zinnia_root_path, "", 1).split("/") if comp]
