...
class TemplateTagsTestCase(TestCase):
    ...
    def test_get_categories_tree(self):
        ...
        self.assertEqual(len(context["categories"]), 0)
        self.assertEqual(context["template"], "zinnia/tags/categories_tree.html")
        self.assertEqual(context["context_category"], None)
        ...
        source_context = Context({"category": category})
        with self.assertNumQueries(0):
            context = get_categories_tree(source_context, "custom_template.html")
        self.assertEqual(len(context["categories"]), 1)
    ...
    def test_get_recent_entries(self):
        ...
        self.assertEqual(context["template"], "zinnia/tags/entries_recent.html")
    ...
    def test_get_featured_entries(self):
        ...
        self.assertEqual(len(context["entries"]), 0)
        self.assertEqual(context["template"], "zinnia/tags/entries_featured.html")
    ...
    def test_draft_entries(self):
        ...
        self.assertEqual(len(context["entries"]), 1)
        self.assertEqual(context["template"], "zinnia/tags/entries_draft.html")
    ...
    def test_get_random_entries(self):
        ...
        self.assertEqual(len(context["entries"]), 0)
        self.assertEqual(context["template"], "zinnia/tags/entries_random.html")
    ...
    def test_get_popular_entries(self):
        ...
        self.assertEqual(len(context["entries"]), 0)
        self.assertEqual(context["template"], "zinnia/tags/entries_popular.html")
    ...
    def test_get_similar_entries(self):
        post_save.connect(flush_similar_cache_handler, sender=Entry, dispatch_uid="flush_cache")
        self.publish_entry()
        source_context = Context({"object": self.entry})
        with self.assertNumQueries(0):
            context = get_similar_entries(source_context)
        self.assertEqual(len(context["entries"]), 0)
        self.assertEqual(context["template"], "zinnia/tags/entries_similar.html")
    ...
    def test_get_archives_entries(self):
        ...
        self.assertEqual(context["archives"][0], self.make_local(self.entry.publication_date).replace(day=1, hour=0))
        self.assertEqual(context["archives"][1], self.make_local(second_entry.publication_date).replace(day=1, hour=0))
        self.assertEqual(context["template"], "custom_template.html")
    ...
    def test_get_archives_tree(self):
        ...
        self.assertEqual(context["archives"][1], self.make_local(self.entry.publication_date).replace(hour=0))
        self.assertEqual(context["template"], "custom_template.html")
    ...
    def test_get_calendar_entries_no_params(self):
        ...
        self.assertEqual(context["template"], "zinnia/tags/entries_calendar.html")

        self.publish_entry()
        with self.assertNumQueries(2):
            context = get_calendar_entries(source_context)
        self.assertEqual(context["previous_month"], self.make_local(self.entry.publication_date).date().replace(day=1))
        self.assertEqual(context["next_month"], None)
    ...
    def test_get_calendar_entries_full_params(self):
        ...
        source_context = Context()
        with self.assertNumQueries(2):
            context = get_calendar_entries(source_context, 2009, 1, template="custom_template.html")
        self.assertEqual(context["previous_month"], None)
        self.assertEqual(context["next_month"], self.make_local(self.entry.publication_date).date().replace(day=1))
        self.assertEqual(context["template"], "custom_template.html")
    ...
    def test_get_calendar_entries_object_context(self):
        ...
        with self.assertNumQueries(2):
            context = get_calendar_entries(source_context)
        self.assertEqual(
            context["previous_month"], self.make_local(second_entry.publication_date).date().replace(day=1)
        )
        self.assertEqual(context["next_month"], None)
    ...
    def test_get_recent_comments(self):
        ...
        self.assertEqual(context["template"], "zinnia/tags/comments_recent.html")

        comment_1 = comments.get_model().objects.create(
            comment="My Comment 1", site=self.site, content_object=self.entry, submit_date=timezone.now()
        )
        with self.assertNumQueries(1):
        ...
        with self.assertNumQueries(3):
            ...
            self.assertEqual(context["comments"][0].content_object, self.entry)

        author = Author.objects.create_user(username="webmaster", email="webmaster@example.com")
        comment_2 = comments.get_model().objects.create(
            comment="My Comment 2", site=self.site, content_object=self.entry, submit_date=timezone.now()
        )
        comment_2.flags.create(user=author, flag=CommentFlag.MODERATOR_APPROVAL)
        with self.assertNumQueries(3):
            context = get_recent_comments()
            self.assertEqual(list(context["comments"]), [comment_2, comment_1])
            self.assertEqual(context["comments"][0].content_object, self.entry)
            self.assertEqual(context["comments"][1].content_object, self.entry)

    @skip_if_custom_user
    def test_get_recent_linkbacks(self):
        user = Author.objects.create_user(username="webmaster", email="webmaster@example.com")
        with self.assertNumQueries(1):
            context = get_recent_linkbacks()
        self.assertEqual(len(context["linkbacks"]), 0)
        self.assertEqual(context["template"], "zinnia/tags/linkbacks_recent.html")

        linkback_1 = comments.get_model().objects.create(
            comment="My Linkback 1", site=self.site, content_object=self.entry, submit_date=timezone.now()
        )
        ...
        with self.assertNumQueries(3):
            ...
            self.assertEqual(context["linkbacks"][0].content_object, self.entry)

        linkback_2 = comments.get_model().objects.create(
            comment="My Linkback 2", site=self.site, content_object=self.entry, submit_date=timezone.now()
        )
        linkback_2.flags.create(user=user, flag=TRACKBACK)
        with self.assertNumQueries(3):
            context = get_recent_linkbacks()
            self.assertEqual(list(context["linkbacks"]), [linkback_2, linkback_1])
            self.assertEqual(context["linkbacks"][0].content_object, self.entry)
            self.assertEqual(context["linkbacks"][1].content_object, self.entry)
    ...
    def test_zinnia_pagination(self):
        ...
        source_context = Context({"request": FakeRequest({"page": "1", "key": "val"})})
        paginator = Paginator(range(200), 10)

        with self.assertNumQueries(0):
            context = zinnia_pagination(
                source_context, paginator.page(1), begin_pages=3, end_pages=3, before_pages=2, after_pages=2
            )
        ...
        with self.assertNumQueries(0):
            context = zinnia_pagination(
                source_context, paginator.page(2), begin_pages=3, end_pages=3, before_pages=2, after_pages=2
            )
        ...
        with self.assertNumQueries(0):
            context = zinnia_pagination(
                source_context, paginator.page(3), begin_pages=3, end_pages=3, before_pages=2, after_pages=2
            )
        ...
        with self.assertNumQueries(0):
            context = zinnia_pagination(
                source_context, paginator.page(6), begin_pages=3, end_pages=3, before_pages=2, after_pages=2
            )
        ...
        with self.assertNumQueries(0):
            context = zinnia_pagination(
                source_context, paginator.page(11), begin_pages=3, end_pages=3, before_pages=2, after_pages=2
            )
        self.assertEqual(list(context["begin"]), [1, 2, 3])
        ...
        with self.assertNumQueries(0):
            context = zinnia_pagination(
                source_context, paginator.page(15), begin_pages=3, end_pages=3, before_pages=2, after_pages=2
            )
        self.assertEqual(list(context["begin"]), [1, 2, 3])
        self.assertEqual(list(context["middle"]), [])
        ...
        with self.assertNumQueries(0):
            context = zinnia_pagination(
                source_context, paginator.page(18), begin_pages=3, end_pages=3, before_pages=2, after_pages=2
            )
        self.assertEqual(list(context["begin"]), [1, 2, 3])
        self.assertEqual(list(context["middle"]), [])
        ...
        with self.assertNumQueries(0):
            context = zinnia_pagination(
                source_context, paginator.page(19), begin_pages=3, end_pages=3, before_pages=2, after_pages=2
            )
        self.assertEqual(list(context["begin"]), [1, 2, 3])
        self.assertEqual(list(context["middle"]), [])
        ...
        with self.assertNumQueries(0):
            context = zinnia_pagination(
                source_context, paginator.page(20), begin_pages=3, end_pages=3, before_pages=2, after_pages=2
            )
        self.assertEqual(list(context["begin"]), [1, 2, 3])
        self.assertEqual(list(context["middle"]), [])
        self.assertEqual(list(context["end"]), [18, 19, 20])
    ...
    def test_zinnia_pagination_on_my_website(self):
        ...
        with self.assertNumQueries(0):
            for i in range(1, 5):
                context = zinnia_pagination(
                    source_context, paginator.page(i), begin_pages=1, end_pages=1, before_pages=2, after_pages=2
                )
    ...
    def test_zinnia_breadcrumbs(self):
        ...
        self.assertEqual(context["breadcrumbs"][0].url, reverse("zinnia:entry_archive_index"))
        self.assertEqual(context["template"], "zinnia/tags/breadcrumbs.html")
        ...
        source_context = Context({"request": FakeRequest(reverse("zinnia:tag_detail", args=["test"])), "object": tag})
        with self.assertNumQueries(0):
            context = zinnia_breadcrumbs(source_context)
        self.assertEqual(len(context["breadcrumbs"]), 2)
        check_only_last_have_no_url(context["breadcrumbs"])
        ...
        source_context = Context({"request": FakeRequest(reverse("zinnia:entry_archive_year", args=[2011]))})
        with self.assertNumQueries(0):
            context = zinnia_breadcrumbs(source_context)
        self.assertEqual(len(context["breadcrumbs"]), 1)
        check_only_last_have_no_url(context["breadcrumbs"])

        source_context = Context({"request": FakeRequest(reverse("zinnia:entry_archive_month", args=[2011, "03"]))})
        with self.assertNumQueries(0):
            context = zinnia_breadcrumbs(source_context)
        self.assertEqual(len(context["breadcrumbs"]), 2)
        check_only_last_have_no_url(context["breadcrumbs"])

        source_context = Context({"request": FakeRequest(reverse("zinnia:entry_archive_week", args=[2011, 15]))})
        with self.assertNumQueries(0):
            context = zinnia_breadcrumbs(source_context)
        self.assertEqual(len(context["breadcrumbs"]), 2)
        check_only_last_have_no_url(context["breadcrumbs"])

        source_context = Context({"request": FakeRequest(reverse("zinnia:entry_archive_day", args=[2011, "03", 15]))})
        with self.assertNumQueries(0):
            context = zinnia_breadcrumbs(source_context)
        self.assertEqual(len(context["breadcrumbs"]), 3)
        check_only_last_have_no_url(context["breadcrumbs"])
        ...
        source_context = Context(
            {
                "request": FakeRequest("%s?page=2" % reverse("zinnia:entry_archive_day", args=[2011, "03", 15])),
                "page_obj": FakePage(2),
            }
        )
        with self.assertNumQueries(0):
        ...
        source_context = Context(
            {
                "request": FakeRequest(reverse("zinnia:entry_archive_day_paginated", args=[2011, "03", 15, 2])),
                "page_obj": FakePage(2),
            }
        )
        with self.assertNumQueries(0):
    ...
    def test_zinnia_loop_template(self):
        ...
        # Test simple
        template = zinnia_loop_template(context, "zinnia/_entry_detail.html")
        self.assertEqual(template.template.name, "zinnia/_entry_detail.html")
        self.assertRaisesRegex(
            TemplateDoesNotExist,
            "zinnia/_entry_custom-0.html, " "zinnia/_entry_custom_0.html, " "zinnia/_entry_custom.html",
        ...
        self.assertRaisesRegex(
            TemplateDoesNotExist,
            "zinnia/_entry_custom-5.html, " "zinnia/_entry_custom_5.html, " "zinnia/_entry_custom.html",
            zinnia_loop_template,
            context,
            "zinnia/_entry_custom.html",
        )
        ...
        context = Context({"forloop": {"counter": 5}, "page_obj": paginator.page(3)})
        self.assertRaisesRegex(
            TemplateDoesNotExist,
            "zinnia/_entry_custom-25.html, " "zinnia/_entry_custom_5.html, " "zinnia/_entry_custom.html",
            zinnia_loop_template,
            context,
            "zinnia/_entry_custom.html",
        )
        ...
        ztemplatetags.ENTRY_LOOP_TEMPLATES = {"default": {25: "template.html"}}
        self.assertRaisesRegex(
            TemplateDoesNotExist,
            "template.html, "
        ...
        # Test with context
        for context_object_name in ["category", "tag", "author", "year", "month", "day"]:
            ztemplatetags.ENTRY_LOOP_TEMPLATES = {
                "slug": {25: "template-slug.html"},
                context_object_name: {25: "template-%s.html" % context_object_name},
                "%s-slug" % context_object_name: {25: "template-%s-slug.html" % context_object_name},
            }
            context = Context({"forloop": {"counter": 5}, "page_obj": paginator.page(3), context_object_name: "slug"})
            self.assertRaisesRegex(
                TemplateDoesNotExist,
                ...
                "zinnia/_entry_custom-25.html, "
                "zinnia/_entry_custom_5.html, "
                "zinnia/_entry_custom.html" % (context_object_name, context_object_name),
                zinnia_loop_template,
                context,
                "zinnia/_entry_custom.html",
            )
    ...
    def test_get_gravatar(self):
        self.assertTrue(
            url_equal(
                ...
                "http://www.gravatar.com/avatar/86d4fd4a22de452" "a9228298731a0b592?s=80&amp;r=g",
            )
        )
        self.assertTrue(
            url_equal(
                get_gravatar("  WEBMASTER@example.com  ", 15, "x", "404"),
                "http://www.gravatar.com/avatar/86d4fd4a22de452" "a9228298731a0b592?s=15&amp;r=x&amp;d=404",
            )
        )
        self.assertTrue(
            url_equal(
                get_gravatar("  WEBMASTER@example.com  ", 15, "x", "404", "https"),
                "https://secure.gravatar.com/avatar/86d4fd4a22de452" "a9228298731a0b592?s=15&amp;r=x&amp;d=404",
            )
        )
    ...
    def test_get_tag_cloud(self):
        ...
        with self.assertNumQueries(1):
            context = get_tag_cloud(source_context, 6, 1, "custom_template.html")
        self.assertEqual(len(context["tags"]), 2)
        self.assertEqual(context["template"], "custom_template.html")
        self.assertEqual(context["context_tag"], tag)

    def test_widont(self):
        self.assertEqual(widont("Word"), "Word")
        self.assertEqual(widont("A complete string"), "A complete&nbsp;string")
        self.assertEqual(widont("A complete\tstring"), "A complete&nbsp;string")
        self.assertEqual(widont("A  complete  string"), "A  complete&nbsp;string")
        self.assertEqual(
            widont("A complete string with trailing spaces  "), "A complete string with trailing&nbsp;spaces  "
        )
        self.assertEqual(
            widont("A complete string with <markup>", autoescape=False), "A complete string with&nbsp;<markup>"
        )
        self.assertEqual(
            widont("A complete string with <markup>", autoescape=True), "A complete string with&nbsp;&lt;markup&gt;"
        )
    ...
    def test_widont_pre_punctuation(self):
        ...
        """
        self.assertEqual(widont("Releases : django-blog-zinnia"), "Releases&nbsp;:&nbsp;django-blog-zinnia")
        self.assertEqual(widont("Releases ; django-blog-zinnia"), "Releases&nbsp;;&nbsp;django-blog-zinnia")
        self.assertEqual(widont("Releases ! django-blog-zinnia"), "Releases&nbsp;!&nbsp;django-blog-zinnia")
        self.assertEqual(widont("Releases ? django-blog-zinnia"), "Releases&nbsp;?&nbsp;django-blog-zinnia")
        self.assertEqual(widont("Releases - django-blog-zinnia"), "Releases&nbsp;-&nbsp;django-blog-zinnia")
        ...
        self.assertEqual(widont("Releases   :   django-blog-zinnia  "), "Releases&nbsp;:&nbsp;django-blog-zinnia  ")
        self.assertEqual(widont("Releases :: django-blog-zinnia"), "Releases&nbsp;::&nbsp;django-blog-zinnia")
        self.assertEqual(widont("Releases :z django-blog-zinnia"), "Releases :z&nbsp;django-blog-zinnia")
    ...
    def test_widont_post_punctuation(self):
        ...
        """
        self.assertEqual(widont("Move !"), "Move&nbsp;!")
        self.assertEqual(widont("Move it   !  "), "Move&nbsp;it&nbsp;!  ")
        self.assertEqual(widont("Move it ?"), "Move&nbsp;it&nbsp;?")
        self.assertEqual(widont("I like to move : it !"), "I like to move&nbsp;:&nbsp;it&nbsp;!")
        self.assertEqual(widont("I like to : move it !"), "I like to : move&nbsp;it&nbsp;!")
...
class TemplateTagsTimezoneTestCase(TestCase):
    ...
    def test_calendar_entries_no_timezone(self):
        template = Template("{% load zinnia %}" "{% get_calendar_entries 2014 1 %}")
    ...
    def test_archives_entries_no_timezone(self):
        template = Template("{% load zinnia %}" "{% get_archives_entries %}")
        self.create_published_entry_at(datetime(2014, 1, 1, 12, 0))
        self.create_published_entry_at(datetime(2014, 1, 31, 23, 0))
        output = template.render(Context())
    ...
    def test_archives_entries_tree_no_timezone(self):
        template = Template("{% load zinnia %}" "{% get_archives_entries_tree %}")
        self.create_published_entry_at(datetime(2014, 1, 1, 12, 0))
        self.create_published_entry_at(datetime(2014, 1, 31, 23, 0))
        output = template.render(Context())
        self.assertTrue("/2014/01/01/" in output)
