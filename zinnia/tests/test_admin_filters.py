...
class BaseListFilterTestCase(TestCase):
    ...
    def setUp(self):
        ...
        activate("en")
        self.root = User.objects.create_superuser("root", "root@exemple.com", "toor")
        ...
        params = {"title": "My entry draft", "content": "My content draft", "slug": "my-entry-draft"}
...
class AuthorListFilterTestCase(BaseListFilterTestCase):
    ...
    def setUp(self):
        ...
        self.authors = [
            Author.objects.create_user(username="webmaster", email="webmaster@example.com"),
            Author.objects.create_user(username="contributor", email="contributor@example.com"),
            Author.objects.create_user(username="reader", email="reader@example.com"),
        ]
    ...
    def test_filter(self):
        ...
        with self.assertNumQueries(1):
            ...
            self.assertEqual(filterspec.used_parameters, {"author": str(self.authors[1].pk)})
            self.assertEqual(
                filterspec.lookup_choices,
                [
                    (str(self.authors[0].pk), "webmaster (2 entries)"),
                    (str(self.authors[1].pk), "contributor (1 entry)"),
                ],
            )
...
class CategoryListFilterTestCase(BaseListFilterTestCase):
    ...
    def setUp(self):
        ...
        self.categories = [
            ...
            Category.objects.create(title="Category 3", slug="cat-3"),
        ]
    ...
    def test_filter(self):
        ...
        request = self.request_factory.get("/", {"category": str(self.categories[1].pk)})
        request.user = self.root
        changelist = self.get_changelist(request, Entry, modeladmin)
        queryset = changelist.get_queryset(request)
        self.assertEqual(queryset.count(), 2)
        ...
        with self.assertNumQueries(1):
            ...
            self.assertEqual(filterspec.used_parameters, {"category": str(self.categories[1].pk)})
            self.assertEqual(
                filterspec.lookup_choices,
                [
                    (str(self.categories[0].pk), "Category 1 (2 entries)"),
                    (str(self.categories[1].pk), "Category 2 (1 entry)"),
                ],
            )
