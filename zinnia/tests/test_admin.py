...
class BaseAdminTestCase(TestCase):
    ...
    def check_with_rich_and_poor_urls(self, func, args, result_rich, result_poor):
...
class EntryAdminTestCase(BaseAdminTestCase):
    ...
    def setUp(self):
        ...
        params = {"title": "My title", "content": "My content", "slug": "my-title"}
        self.entry = Entry.objects.create(**params)
        self.request_factory = RequestFactory()
        self.request = self.request_factory.get("/")

    def test_get_title(self):
        self.assertEqual(self.admin.get_title(self.entry), "My title (2 words)")
        self.entry.comment_count = 1
        self.entry.save()
        self.entry = Entry.objects.get(pk=self.entry.pk)
        self.assertEqual(self.admin.get_title(self.entry), "My title (2 words) (1 reaction)")
        self.entry.pingback_count = 1
        self.entry.save()
        self.entry = Entry.objects.get(pk=self.entry.pk)
        self.assertEqual(self.admin.get_title(self.entry), "My title (2 words) (2 reactions)")

    def test_get_authors(self):
        self.check_with_rich_and_poor_urls(self.admin.get_authors, (self.entry,), "", "")
        author_1 = Author.objects.create_user("author-1", "author1@example.com")
        author_2 = Author.objects.create_user("author<2>", "author2@example.com")
        self.entry.authors.add(author_1)
        self.check_with_rich_and_poor_urls(
            self.admin.get_authors,
            (self.entry,),
    ...
    def test_get_authors_non_ascii(self):
        author = Author.objects.create_user("тест", "test@example.com")
        self.entry.authors.add(author)
        self.check_with_rich_and_poor_urls(
            self.admin.get_authors,
            (self.entry,),
            '<a href="/authors/%D1%82%D0%B5%D1%81%D1%82/" ' 'target="blank">тест</a>',
            "тест",
        )

    def test_get_categories(self):
        self.check_with_rich_and_poor_urls(self.admin.get_categories, (self.entry,), "", "")
        category_1 = Category.objects.create(title="Category <b>1</b>", slug="category-1")
        category_2 = Category.objects.create(title="Category <b>2</b>", slug="category-2")
        self.entry.categories.add(category_1)
        self.check_with_rich_and_poor_urls(
            self.admin.get_categories,
            (self.entry,),
    ...
    def test_get_categories_non_ascii(self):
        category = Category.objects.create(title="Category тест", slug="category")
        self.entry.categories.add(category)
        self.check_with_rich_and_poor_urls(
            self.admin.get_categories,
            (self.entry,),
            '<a href="/categories/category/" target="blank">' "Category тест</a>",
            "Category тест",
        )

    def test_get_tags(self):
        self.check_with_rich_and_poor_urls(self.admin.get_tags, (self.entry,), "", "")
        self.entry.tags = "zinnia"
        self.check_with_rich_and_poor_urls(
            self.admin.get_tags, (self.entry,), '<a href="/tags/zinnia/" target="blank">zinnia</a>', "zinnia"
        )
        ...
        self.check_with_rich_and_poor_urls(
            ...
            "zinnia, t&lt;e&gt;st",
        )  # Yes, this is not the same order...
    ...
    def test_get_tags_non_ascii(self):
        ...
        self.check_with_rich_and_poor_urls(
            self.admin.get_tags,
            (self.entry,),
            '<a href="/tags/%D1%82%D0%B5%D1%81%D1%82/" ' 'target="blank">тест</a>',
            "тест",
        )
    ...
    def test_get_is_visible(self):
        self.assertEqual(self.admin.get_is_visible(self.entry), self.entry.is_visible)

    def test_queryset(self):
        user = Author.objects.create_user("user", "user@exemple.com")
        self.entry.authors.add(user)
        root = Author.objects.create_superuser("root", "root@exemple.com", "toor")
        params = {"title": "My root title", "content": "My root content", "slug": "my-root-titile"}
    ...
    def test_get_changeform_initial_data(self):
        user = User.objects.create_user("user", "user@exemple.com")
        site = Site.objects.get_current()
        self.request.user = user
        data = self.admin.get_changeform_initial_data(self.request)
        self.assertEqual(data, {"authors": [user.pk], "sites": [site.pk]})
    ...
    def test_formfield_for_manytomany(self):
        staff = User.objects.create_user("staff", "staff@exemple.com")
        author = User.objects.create_user("author", "author@exemple.com")
        root = User.objects.create_superuser("root", "root@exemple.com", "toor")
        self.request.user = root
        field = self.admin.formfield_for_manytomany(Entry.authors.field, self.request)
    ...
    def test_get_readonly_fields(self):
        ...
        root = User.objects.create_superuser("root", "root@exemple.com", "toor")
        self.request.user = user
        self.assertEqual(self.admin.get_readonly_fields(self.request), ["status", "authors"])
        self.request.user = root
    ...
    def test_get_actions_in_popup_mode_issue_291(self):
        ...
        request.user = user
        self.assertEqual(list(self.admin.get_actions(request).keys()), [])
    ...
    def test_ping_directories(self):
        class FakePinger(object):
            def __init__(self, *ka, **kw):
                self.results = [{"flerror": False, "message": "OK"}, {"flerror": True, "message": "KO"}]
        ...
        self.assertEqual(
            ...
            [
                ...
                (20, "http://ping.com/ping directory successfully " "pinged 1 entries.", ""),
            ],
        )
...
class CategoryAdminTestCase(BaseAdminTestCase):
    ...
    def test_get_tree_path(self):
        ...
        self.check_with_rich_and_poor_urls(
            self.admin.get_tree_path, (category,), '<a href="/categories/cat/" target="blank">/cat/</a>', "/cat/"
        )
...
class FunctionnalAdminTestCase(TestCase):
    ...
    def setUp(self):
        disconnect_entry_signals()
        self.author = Author.objects.create_superuser(username="admin", email="admin@example.com", password="password")

        self.category = Category.objects.create(title="Category", slug="cat")
        params = {"title": "My title", "content": "My content", "slug": "my-title"}
        self.entry = Entry.objects.create(**params)
    ...
    def test_admin_entry_list(self):
        self.assert_admin(reverse("admin:zinnia_entry_changelist"))

    def test_admin_category_list(self):
        self.assert_admin(reverse("admin:zinnia_category_changelist"))

    def test_admin_entry_add(self):
        self.assert_admin(reverse("admin:zinnia_entry_add"))

    def test_admin_category_add(self):
        self.assert_admin(reverse("admin:zinnia_category_add"))

    def test_admin_entry_update(self):
        self.assert_admin(reverse("admin:zinnia_entry_change", args=[self.entry.pk]))

    def test_admin_category_update(self):
        self.assert_admin(reverse("admin:zinnia_category_change", args=[self.category.pk]))
