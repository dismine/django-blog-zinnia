...
class PingBackTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.author = Author.objects.create_user(username="webmaster", email="webmaster@example.com")
    ...
    def test_generate_pingback_content(self):
        ...
        soup = BeautifulSoup('<a href="%s">test link</a>' % target, "html.parser")
        self.assertEqual(generate_pingback_content(soup, target, 6), "test l...")

        soup = BeautifulSoup('test <a href="%s">link</a>' % target, "html.parser")
    ...
    def test_pingback_ping(self):
        ...
        response = self.server.pingback.ping("http://%s/" % self.site.domain, "http://%s/" % self.site.domain)
        self.assertEqual(response, 0)
        ...
        response = self.server.pingback.ping(source, "http://example.com/error-404/")
