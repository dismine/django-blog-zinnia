...
import functools
from datetime import datetime as original_datetime, timezone as datetime_timezone
...
def url_equal(url_1, url_2):
    ...
    return parse_result_1[:4] == parse_result_2[:4] and parse_qs(parse_result_1[5]) == parse_qs(parse_result_2[5])
...
class VoidLoader(Loader):
    ...
    def get_template_sources(self, template_name):
        yield Origin(name="voidloader", template_name=template_name, loader=self)
...
class EntryDetailLoader(Loader):
    ...
    def get_template_sources(self, template_name):
        yield Origin(name="entrydetailloader", template_name=template_name, loader=self)

    def get_contents(self, origin):
        return (
            "<html><head><title>{{ object.title }}</title></head>" "<body>{{ object.html_content|safe }}</body></html>"
        )
