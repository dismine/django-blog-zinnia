...
class ViewsBaseCase(TestCase):
    ...
    def setUp(self):
        ...
        self.author = Author.objects.create_user(username="admin", email="admin@example.com", password="password")
    ...
    def check_publishing_context(self, url, first_expected, second_expected=None, friendly_context=None, queries=None):
        ...
        if queries is not None:
            ...
            response = self.client.get(url)
        self.assertEqual(len(response.context["object_list"]), first_expected)
        if second_expected:
            self.create_published_entry()
            response = self.client.get(url)
            self.assertEqual(len(response.context["object_list"]), second_expected)
        if friendly_context:
            self.assertEqual(response.context["object_list"], response.context[friendly_context])
        return response
...
@override_settings(ROOT_URLCONF="zinnia.tests.implementations.urls.default")
class ViewsTestCase(ViewsBaseCase):
    ...
    def test_zinnia_entry_archive_year_no_timezone(self):
        response = self.check_publishing_context("/2010/", 2, 3, "entry_list", 3)
        self.assertTemplateUsed(response, "zinnia/archives/2010/entry_archive_year.html")
    ...
    def test_zinnia_entry_archive_week_no_timezone(self):
        response = self.check_publishing_context("/2010/week/00/", 1, 2, "entry_list", 3)
        self.assertTemplateUsed(response, "zinnia/archives/2010/week/0/entry_archive_week.html")
    ...
    def test_zinnia_entry_archive_week_with_timezone(self):
        response = self.check_publishing_context("/2010/week/0/", 1, 2, "entry_list", 3)
        self.assertTemplateUsed(response, "zinnia/archives/2010/week/0/entry_archive_week.html")
        # All days in a new year preceding the first Monday
        # are considered to be in week 0.
        self.assertEqual(response.context["week"], date(2009, 12, 28))
    ...
    def test_zinnia_entry_archive_month_no_timezone(self):
        response = self.check_publishing_context("/2010/01/", 1, 2, "entry_list", 3)
        self.assertTemplateUsed(response, "zinnia/archives/2010/month/1/entry_archive_month.html")
        ...
        self.assertEqual(list(response.context_data["date_list"]), [datetime(2010, 1, 1)])
        ...
        self.assertEqual(list(response.context_data["date_list"]), [datetime(2010, 5, 31)])
    ...
    def test_zinnia_entry_archive_day_no_timezone(self):
        response = self.check_publishing_context("/2010/01/01/", 1, 2, "entry_list", 2)
        self.assertTemplateUsed(response, "zinnia/archives/2010/1/1/entry_archive_day.html")
        self.assertEqual(response.context_data["previous_month"], None)
        self.assertEqual(response.context_data["next_month"], date(2010, 5, 1))
    ...
    def test_zinnia_entry_archive_day_with_timezone(self):
        response = self.check_publishing_context("/2010/01/02/", 1, 2, "entry_list", 2)
        self.assertTemplateUsed(response, "zinnia/archives/2010/1/2/entry_archive_day.html")
        self.assertEqual(response.context["previous_month"], None)
        self.assertEqual(response.context["next_month"], date(2010, 6, 1))
    ...
    def test_zinnia_entry_archive_today_with_timezone(self):
        template_name_today = (
            f"zinnia/archives/{timezone.localtime(timezone.now()).strftime('%Y/%m/%d')}/entry_archive_today.html"
        )

        with self.assertNumQueries(2):
            response = self.client.get("/today/")
        self.assertTemplateUsed(response, template_name_today)
        self.assertEqual(response.context["day"], timezone.localtime(timezone.now()).date())
    ...
    def test_zinnia_entry_shortlink(self):
        ...
        self.assertEqual(response["Location"], self.first_entry.get_absolute_url())
    ...
    def test_zinnia_entry_detail_no_timezone(self):
        ...
        with self.assertNumQueries(1):
            response = self.client.get(entry.get_absolute_url())
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "zinnia/archives/2010/1/1/my-test-entry_entry_custom.html")
    ...
    def test_zinnia_entry_detail_with_timezone(self):
        ...
        with self.assertNumQueries(1):
            response = self.client.get(entry.get_absolute_url())
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "zinnia/archives/2010/1/2/my-test-entry_entry_custom.html")
    ...
    def test_zinnia_entry_detail_login(self):
        ...
        with self.assertNumQueries(3):
            response = self.client.post(entry.get_absolute_url(), {"username": "admin", "password": "password"})
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "zinnia/archives/2010/1/1/my-test-entry_entry_detail.html")
    ...
    def test_zinnia_entry_detail_password(self):
        ...
        with self.assertNumQueries(1):
            response = self.client.post(entry.get_absolute_url(), {"entry_password": "bad_password"})
        self.assertTemplateUsed(response, "zinnia/password.html")
        self.assertEqual(response.context["error"], True)
        with self.assertNumQueries(1):
            response = self.client.post(entry.get_absolute_url(), {"entry_password": "password"})
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "zinnia/archives/2010/1/1/my-test-entry_entry_detail.html")
    ...
    def test_zinnia_entry_detail_preview(self):
        ...
        with self.assertNumQueries(2):
            response = self.client.get(url)
        self.assertEqual(response.status_code, 404)
        Author.objects.create_superuser("root", "root@example.com", "password")
    ...
    def test_zinnia_category_list(self):
        category = Category.objects.create(title="New category", slug="new-category")
    ...
    def test_zinnia_category_detail(self):
        response = self.check_publishing_context("/categories/tests/", 2, 3, "entry_list", 2)
        self.assertTemplateUsed(response, "zinnia/category/tests/entry_list.html")
    ...
    def test_zinnia_author_list(self):
        user = Author.objects.create(username="new-user", email="new_user@example.com")
    ...
    def test_zinnia_author_detail(self):
        response = self.check_publishing_context("/authors/admin/", 2, 3, "entry_list", 2)
        self.assertTemplateUsed(response, "zinnia/author/admin/entry_list.html")
    ...
    def test_zinnia_entry_search(self):
        self.check_publishing_context("/search/?pattern=test", 2, 3, "entry_list", 1)
        response = self.client.get("/search/?pattern=ab")
        self.assertEqual(len(response.context["object_list"]), 0)
        self.assertEqual(response.context["error"], _("The pattern is too short"))
        response = self.client.get("/search/")
        self.assertEqual(len(response.context["object_list"]), 0)
        self.assertEqual(response.context["error"], _("No pattern to search found"))
    ...
    def test_zinnia_trackback(self):
        ...
        response = self.client.post(trackback_url, {"url": "http://example.com"})
        self.assertEqual(response["Content-Type"], "text/xml")
        self.assertEqual(response.context["error"], "Trackback is not enabled for Test 1")
        ...
        response = self.client.post(trackback_url, {"url": "http://example.com"})
        self.assertEqual(response.context["error"], "Trackback is already registered")
    ...
    def test_zinnia_trackback_on_entry_without_author(self):
        ...
        response = self.client.post("/trackback/%s/" % self.first_entry.pk, {"url": "http://example.com"})
        self.assertEqual(response["Content-Type"], "text/xml")
        self.assertEqual("error" in response.context, False)
        zinnia.spam_checker.SPAM_CHECKER_BACKENDS = original_scb
    ...
    def test_zinnia_trackback_spam_check(self):
        ...
        original_scb = zinnia.spam_checker.SPAM_CHECKER_BACKENDS
        zinnia.spam_checker.SPAM_CHECKER_BACKENDS = ("zinnia.spam_checker.backends.all_is_spam",)
        response = self.client.post(
            "/trackback/%s/" % self.first_entry.pk, {"url": "http://example.com", "excerpt": "Spam"}
        )
        self.assertEqual(response["Content-Type"], "text/xml")
        self.assertEqual(response.context["error"], "Trackback considered like spam")
        zinnia.spam_checker.SPAM_CHECKER_BACKENDS = []
        response = self.client.post(
            "/trackback/%s/" % self.first_entry.pk, {"url": "http://example.com", "excerpt": "Spam"}
        )
    ...
    def test_capabilities(self):
        ...
        self.check_capabilities("/wlwmanifest.xml", "application/wlwmanifest+xml", 0)
        self.check_capabilities("/opensearch.xml", "application/opensearchdescription+xml", 0)
    ...
    def test_comment_success(self):
        ...
        self.assertEqual(response.redirect_chain[1], ("http://example.com/categories/tests/", 302))
    ...
    def test_quick_entry(self):
        ...
        self.assertEqual(response["Location"], "/accounts/login/?next=/quick-entry/")
        ...
        self.assertTrue(
            url_equal(
                response["Location"],
                "/admin/zinnia/entry/add/" "?tags=&title=&sites=1&content=" "%3Cp%3Etest%3C%2Fp%3E&authors=2&slug=",
            )
        )
        response = self.client.post(
            "/quick-entry/", {"title": "test", "tags": "test", "content": "Test content", "save_draft": ""}
        )
    ...
    def test_quick_entry_non_ascii_title_issue_153(self):
        ...
        self.client.login(username="root", password="password")
        response = self.client.post(
            "/quick-entry/", {"title": "тест", "tags": "test-2", "content": "Test content", "save_draft": ""}
        )
        self.assertEqual(response.status_code, 302)
        self.assertTrue(
            url_equal(
    ...
    def test_quick_entry_markup_language_issue_270(self):
        ...
        response = self.client.post(
            "/quick-entry/",
            {"title": "Test markup", "tags": "test, markup", "content": "Hello *World* !", "save_draft": ""},
        )
...
@override_settings(ROOT_URLCONF="zinnia.tests.implementations.urls.custom_detail_views")
