...
class MetaWeblogTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.webmaster = Author.objects.create_superuser(
            username="webmaster", email="webmaster@example.com", password="password"
        )
        self.contributor = Author.objects.create_user(
            username="contributor", email="contributor@example.com", password="password"
        )
    ...
    def test_get_users_blogs(self):
        self.assertRaises(Fault, self.server.blogger.getUsersBlogs, "apikey", "contributor", "password")
        self.assertEqual(
            self.server.blogger.getUsersBlogs("apikey", "webmaster", "password"),
            [{"url": "http://example.com/", "blogid": 1, "blogName": "example.com"}],
        )

    def test_get_user_info(self):
        self.assertRaises(Fault, self.server.blogger.getUserInfo, "apikey", "contributor", "password")
        self.webmaster.first_name = "John"
        self.webmaster.save()
        self.assertEqual(
            self.server.blogger.getUserInfo("apikey", "webmaster", "password"),
            {
                "firstname": "John",
                "lastname": "",
    ...
    def test_get_authors(self):
        self.assertRaises(Fault, self.server.wp.getAuthors, "apikey", "contributor", "password")
        self.assertEqual(
            self.server.wp.getAuthors("apikey", "webmaster", "password"),
            [
                {
    ...
    def test_get_tags(self):
        self.assertRaises(Fault, self.server.wp.getTags, 1, "contributor", "password")
        self.assertEqual(
    ...
    def test_get_categories(self):
        self.assertRaises(Fault, self.server.metaWeblog.getCategories, 1, "contributor", "password")
        self.assertEqual(
            self.server.metaWeblog.getCategories("apikey", "webmaster", "password"),
            [
                {
                    "rssUrl": "http://example.com/feeds/categories/category-1/",
                    "description": "Category 1",
                    "htmlUrl": "http://example.com/categories/category-1/",
                    "categoryId": self.categories[0].pk,
                    "parentId": 0,
    ...
    def test_new_category(self):
        ...
        self.assertRaises(Fault, self.server.wp.newCategory, 1, "contributor", "password", category_struct)
        self.assertEqual(Category.objects.count(), 2)
        new_category_id = self.server.wp.newCategory(1, "webmaster", "password", category_struct)
    ...
    def test_get_recent_posts(self):
        self.assertRaises(Fault, self.server.metaWeblog.getRecentPosts, 1, "contributor", "password", 10)
        self.assertEqual(len(self.server.metaWeblog.getRecentPosts(1, "webmaster", "password", 10)), 2)

    def test_delete_post(self):
        self.assertRaises(Fault, self.server.blogger.deletePost, "apikey", 1, "contributor", "password", "publish")
        self.assertEqual(Entry.objects.count(), 2)
        self.assertTrue(self.server.blogger.deletePost("apikey", self.entry_1.pk, "webmaster", "password", "publish"))
        self.assertEqual(Entry.objects.count(), 1)

    def test_get_post(self):
        self.assertRaises(Fault, self.server.metaWeblog.getPost, 1, "contributor", "password")
        post = self.server.metaWeblog.getPost(self.entry_1.pk, "webmaster", "password")
        ...
        self.assertEqual(post["link"], "http://example.com/2010/01/01/my-entry-1/")
        self.assertEqual(post["permaLink"], "http://example.com/2010/01/01/my-entry-1/")
    ...
    def test_new_media_object(self):
        ...
        file_.seek(0)
        media = {"name": "test file.txt", "type": "text/plain", "bits": Binary(file_.read())}
        file_.close()

        self.assertRaises(Fault, self.server.metaWeblog.newMediaObject, 1, "contributor", "password", media)
        new_media = self.server.metaWeblog.newMediaObject(1, "webmaster", "password", media)
