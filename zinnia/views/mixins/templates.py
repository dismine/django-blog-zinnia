...
class EntryQuerysetTemplateResponseMixin(TemplateResponseMixin):
    ...
    def get_model_type(self):
        ...
        if self.model_type is None:
            raise ImproperlyConfigured(
                ...
                "'model_type' or an implementation of 'get_model_type()'" % self.__class__.__name__
            )
    ...
    def get_model_name(self):
        ...
        if self.model_name is None:
            raise ImproperlyConfigured(
                "%s requires either a definition of "
                "'model_name' or an implementation of 'get_model_name()'" % self.__class__.__name__
            )
...
class EntryQuerysetArchiveTemplateResponseMixin(TemplateResponseMixin):
    ...
    def get_template_names(self):
        ...
        for template_name in template_names:
            templates.extend([template_name, f"zinnia/{template_name}", f"{path}/{template_name}"])
        if year is not None:
            templates.extend(f"{path}/{year}/{template_name}" for template_name in template_names)

        if week is not None:
            for template_name in template_names:
                templates.extend([f"{path}/week/{week}/{template_name}", f"{path}/{year}/week/{week}/{template_name}"])

        if month is not None:
            for template_name in template_names:
                templates.extend(
                    [f"{path}/month/{month}/{template_name}", f"{path}/{year}/month/{month}/{template_name}"]
                )
...
class EntryQuerysetArchiveTodayTemplateResponseMixin(EntryQuerysetArchiveTemplateResponseMixin):
    """
...
class EntryArchiveTemplateResponseMixin(EntryQuerysetArchiveTemplateResponseMixin):
    """
    Same as EntryQuerysetArchiveTemplateResponseMixin
