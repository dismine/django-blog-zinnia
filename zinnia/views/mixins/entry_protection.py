...
class PasswordMixin(object):
    ...
    def password(self):
        ...
        return self.response_class(
            request=self.request, template="zinnia/password.html", context={"error": self.error}
        )
...
class EntryProtectionMixin(LoginMixin, PasswordMixin):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if self.object.login_required and not request.user.is_authenticated:
            return self.login()
        if self.object.password and self.object.password != self.request.session.get(
            self.session_key % self.object.pk
        ):
