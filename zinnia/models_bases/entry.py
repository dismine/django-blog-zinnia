...
class CoreEntry(models.Model):
    ...
    STATUS_CHOICES = ((DRAFT, _("draft")), (HIDDEN, _("hidden")), (PUBLISHED, _("published")))

    title = models.CharField(_("title"), max_length=255)

    slug = models.SlugField(
        _("slug"), max_length=255, unique_for_date="publication_date", help_text=_("Used to build the entry's URL.")
    )

    status = models.IntegerField(_("status"), db_index=True, choices=STATUS_CHOICES, default=DRAFT)

    publication_date = models.DateTimeField(
        _("publication date"), db_index=True, default=timezone.now, help_text=_("Used to build the entry's URL.")
    )

    start_publication = models.DateTimeField(
        _("start publication"), db_index=True, blank=True, null=True, help_text=_("Start date of publication.")
    )

    end_publication = models.DateTimeField(
        _("end publication"), db_index=True, blank=True, null=True, help_text=_("End date of publication.")
    )

    sites = models.ManyToManyField(
        Site, related_name="entries", verbose_name=_("sites"), help_text=_("Sites where the entry will be published.")
    )
...
class DiscussionsEntry(models.Model):
    ...
    pingback_enabled = models.BooleanField(
        _("pingbacks enabled"), default=True, help_text=_("Allows pingbacks if checked.")
    )
    trackback_enabled = models.BooleanField(
        _("trackbacks enabled"), default=True, help_text=_("Allows trackbacks if checked.")
    )
...
class ImageEntry(models.Model):
    ...
    image = models.ImageField(
        _("image"), blank=True, upload_to=image_upload_to_dispatcher, help_text=_("Used for illustration.")
    )
...
class CategoriesEntry(models.Model):
    ...
    categories = models.ManyToManyField(
        "zinnia.Category", blank=True, related_name="entries", verbose_name=_("categories")
    )
...
class LoginRequiredEntry(models.Model):
    ...
    login_required = models.BooleanField(
        _("login required"), default=False, help_text=_("Only authenticated users can view the entry.")
    )
...
class PasswordRequiredEntry(models.Model):
    ...
    password = models.CharField(
        _("password"), max_length=50, blank=True, help_text=_("Protects the entry with a password.")
    )
