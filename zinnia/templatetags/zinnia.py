...
END_PUNCTUATION_WIDONT_REGEXP = re.compile(r"\s+([?!]+\s*)$")
...
def get_categories(context, template="zinnia/tags/categories.html"):
    ...
    return {
        "template": template,
        "categories": Category.published.all().order_by("title").annotate(count_entries_published=Count("entries")),
...
def get_categories_tree(context, template="zinnia/tags/categories_tree.html"):
    ...
    return {
        "template": template,
        "categories": Category.objects.all().annotate(count_entries=Count("entries")),
        "context_category": context.get("category"),
    }
...
def get_authors(context, template="zinnia/tags/authors.html"):
    ...
    return {
        "template": template,
        "authors": Author.published.all().annotate(count_entries_published=Count("entries")),
...
def get_recent_entries(number=5, template="zinnia/tags/entries_recent.html"):
    ...
    """
    return {"template": template, "entries": Entry.published.all()[:number]}
...
@register.inclusion_tag("zinnia/tags/dummy.html")
def get_draft_entries(number=5, template="zinnia/tags/entries_draft.html"):
    """
...
def get_popular_entries(number=5, template="zinnia/tags/entries_popular.html"):
    ...
    return {
        "template": template,
        "entries": Entry.published.filter(comment_count__gt=0).order_by("-comment_count", "-publication_date")[
            :number
        ],
    }
...
@register.inclusion_tag("zinnia/tags/dummy.html")
def get_archives_entries_tree(template="zinnia/tags/entries_archives_tree.html"):
    """
...
def get_calendar_entries(context, year=None, month=None, template="zinnia/tags/entries_calendar.html"):
    ...
    return {
        ...
        "calendar": calendar.formatmonth(
            current_month.year, current_month.month, previous_month=previous_month, next_month=next_month
        ),
    }
...
def get_recent_comments(number=5, template="zinnia/tags/comments_recent.html"):
    ...
    entry_published_pks = map(smart_str, Entry.published.values_list("id", flat=True))
...
@register.inclusion_tag("zinnia/tags/dummy.html", takes_context=True)
def zinnia_pagination(
    context, page, begin_pages=1, end_pages=1, before_pages=2, after_pages=2, template="zinnia/tags/pagination.html"
):
    """
    Return a Digg-like pagination,
    by splitting long list of page into 3 blocks of pages.
    """
    get_string = "".join(f"&{key}={value}" for key, value in context["request"].GET.items() if key != "page")
    page_range = list(page.paginator.page_range)
    begin = page_range[:begin_pages]
    end = page_range[-end_pages:]
    middle = page_range[max(page.number - before_pages - 1, 0) : page.number + after_pages]  # noqa: E203
    ...
    return {"template": template, "page": page, "begin": begin, "middle": middle, "end": end, "GET_string": get_string}
...
def zinnia_loop_template(context, default_template):
    ...
    matching, context_object = get_context_first_matching_object(
        context, ["category", "tag", "author", "pattern", "year", "month", "week", "day"]
    )
...
@register.simple_tag
def get_gravatar(email, size=80, rating="g", default=None, protocol=PROTOCOL):
    """
    Return url for a Gravatar.
    """
    gravatar_protocols = {"http": "http://www", "https": "https://secure"}
    url = (
        f"{gravatar_protocols[protocol]}.gravatar.com/avatar/{md5(email.strip().lower().encode('utf-8')).hexdigest()}"
    )
...
def get_tags():
    ...
    """
    return Tag.objects.usage_for_queryset(Entry.published.all())
