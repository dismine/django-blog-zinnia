...
def generate_pingback_content(soup, target, max_length, trunc_char="..."):
    ...
    if len(content) > max_length:
        return _extract_pingback_content(max_length, index, content, trunc_char)
    return content
...
def pingback_ping(source, target):
    ...
    try:
        ...
        pingback_defaults = {"comment": pingback_datas.pop("comment"), "user_name": pingback_datas.pop("user_name")}
        pingback, created = pingback_klass.objects.get_or_create(defaults=pingback_defaults, **pingback_datas)
        if created:
